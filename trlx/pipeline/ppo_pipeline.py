...
from trlx.data.ppo_types import PPORLBatch, PPORLElement, PPORLBatchWithLogits, PPORLElementWithLogits
...
class PPORolloutStorage(BaseRolloutStore):
    ...
    def create_loader(
        ...
        def collate_fn(elems):
            if isinstance(elems[0], PPORLElement):
                return PPORLBatch(
            ...
            else:  # PPORLElementWithLogits
                return PPORLBatchWithLogits(
                    # Left padding of already left-padded queries
                    pad_sequence(
                        [elem.query_tensor.flip(0) for elem in elems],
                        padding_value=self.pad_token_id,
